...
"""
md_to_docx.py
将专利 Markdown 文件转换为格式规范的 Word DOCX。
自动发现 patents/ 目录下的所有专利，支持按名称过滤。

用法:
  python tools/md_to_docx.py              # 转换所有专利
  python tools/md_to_docx.py patent_06    # 只转换 patent_06
"""
...
def discover_patents(filter_name=None):
    """自动发现 patents/ 下所有 patent_*_disclosure.md，返回 (md_path, docx_path) 列表。
    
    filter_name: 可选，如 'patent_06' 只处理匹配的专利。
    """
    pattern = os.path.join(os.path.abspath(PATENTS_DIR), 'patent_*', 'patent_*_disclosure.md')
    md_files = sorted(glob.glob(pattern))
    patents = []
    for md_path in md_files:
        if filter_name and filter_name not in os.path.basename(os.path.dirname(md_path)):
            continue
        docx_path = md_path.replace('_disclosure.md', '_disclosure.docx')
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description='专利 Markdown → DOCX 批量转换')
